...
class _OrdersScreenState extends ConsumerState<OrdersScreen> {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (isSelectionMode) _buildSelectionControlBar(selectedOrders),

              const Gap(5),
              
              // العنوان مع زر التحديد المتعدد
              _buildHeaderWithSelectionButton(isSelectionMode),
              
              // المحتوى
              _buildOrdersList(ordersState, isSelectionMode),
              
              // زر إنشاء الشحنة
              if (isSelectionMode && selectedOrders.isNotEmpty)
                _buildCreateShipmentButton(selectedOrders.length, isCreatingShipment),
            ],
          ),
        ),
      ),
  ...
  Widget _buildSelectionControlBar(Set<String> selectedOrders) {
    return Container(
  ...
  Widget _buildHeaderWithSelectionButton(bool isSelectionMode) {
    return Padding(
      padding: const EdgeInsets.all(8.0),
      child: Row(
        children: [
  ...
  Widget _buildOrdersList(AsyncValue<List<Order>> ordersState, bool isSelectionMode) {
    return ordersState.when(
      ...
      error: (err, _) => Expanded(
        child: Center(child: Text('خطأ: ${err.toString()}')),
      ),
    );
  }

  Widget _buildCreateShipmentButton(int selectedCount, bool isCreatingShipment) {
    return Container(
